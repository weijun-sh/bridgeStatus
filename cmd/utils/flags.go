...
var (
	...
	// PriceFileFlag -p|--price
	PriceFileFlag = &cli.StringFlag{
		Name:    "price",
		Aliases: []string{"p"},
		Usage:   "Specify price config file",
	}
	// LogFileFlag --log
...
// GetPriceFilePath specified by `-p|--price`
func GetPriceFilePath(ctx *cli.Context) string {
	return ctx.String(PriceFileFlag.Name)
}
